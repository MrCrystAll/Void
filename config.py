...
class Configuration:
    def __init__(self, state_setter, terminal_conditions, rewards, action_parser: ActionParser = DiscreteAction(), obs_builder: ObsBuilder = ExpandAdvancedObs(),team_size: int = 3, spawn_opponents: bool = True, **kwargs):
...
version_dict = {
    "default": Configuration(
        state_setter=[[CustomStateSetter(),DefaultState(),ShotState(),SaveState(),AerialBallState()],[1, 1, 49, 49, 20]],
        terminal_conditions=[[TimeoutCondition(fps * 300),NoTouchTimeoutCondition(fps * 45),GoalScoredCondition()]],
        rewards=[[EventReward(goal=100,concede=-100,touch=10,save=50,shot=50),FaceBallReward(),LiuDistanceBallToGoalReward(),VelocityBallToGoalReward()],[1.0, 0.7, 1.4]]
    ),
    "aerial": Configuration(
        state_setter=[[AerialBallState(), StandingBallState(), AirDribble2Touch()], [24, 3, 24]],
        terminal_conditions=[TimeoutCondition(fps * 300),NoTouchTimeoutCondition(fps * 40),GoalScoredCondition(),BallGroundCondition()],
    ...
    "recovery": Configuration(
        ...
        rewards=[[TouchBallReward(), VelocityPlayerToBallReward(), VelocityBallToGoalReward(), EpisodeLengthReward(), DistancePlayerToBall()], [10, 3, .2, .03, .1]],
        team_size=1,
